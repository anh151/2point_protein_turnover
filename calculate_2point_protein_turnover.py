...
def prompt():
    ...
    # folder_name = 'C:\\Users\\ANDREW\\Desktop\\Sad16_2point_1iso'
    # fsr_selection = '1'
    # number_of_zeros = 2
    # number_of_T2 = 2
    # bwe = '0,0,2.5,2.5'
    # user_time_points = '0,0,168,168'
...
def read_data_A4_FSR(file, file_name, file_path, folder_name, fsr_selection, number_of_zeros, number_of_T2, bwe, user_time_points):
	...
	with open(file_path) as file:
		...
		final_data_df = organize_final_data(file_name, final_data_df)
		final_data_df.to_excel(final_file_path, index = False)

def organize_final_data(file_name, final_data_df):
	'''Need to fix how I am creating the dataframe
	Need to first create a copy of the original dataframe with the data unmodified
	Modify this copy by removing the bad data
	Then concat the unmodified and modified dataframes together
	Then add all of the formulas. These should be the same and I shouldnt have to modify anything'''
	from openpyxl.utils import get_column_letter

	removed_bad_data = final_data_df#created a working copy to remove the bad data
	temp_df = removed_bad_data.iloc[:, 1:]#Get only the numbers
	temp = set()
	for index, row in removed_bad_data.iterrows():
		for value in row:
			if check_number(value):#ignores any strings
				temp.add(value)
	...
	try:
		...
		mean = 0
	removed_bad_data = removed_bad_data.apply(pd.to_numeric, errors='coerce')
	for column in removed_bad_data.columns:
		removed_bad_data.loc[removed_bad_data[column]>1.4*mean, column] = ''
		removed_bad_data = removed_bad_data.apply(pd.to_numeric, errors='coerce')
		removed_bad_data.loc[removed_bad_data[column]<0.6*mean, column] = ''

	removed_bad_data.drop(columns=['peptide'], inplace=True)
	removed_bad_data = pd.concat([final_data_df['peptide'], removed_bad_data], axis=1)
	final_data_df = pd.concat([final_data_df, removed_bad_data])#creates the final data frame that is the combo of 2 dataframes
	number_of_rows = removed_bad_data.shape[0]
	temp_empty = ['' for i in range(number_of_rows-1)]

	average_part_one = '=average(E2:{}{})'.format(get_column_letter(final_data_df.shape[1] + 3), 1 + number_of_rows)
	average_part_two = '=average(E{}:{}{})'.format(number_of_rows + 2, get_column_letter(final_data_df.shape[1] + 3), number_of_rows*2 + 1)

	cv_part_one = '=stdev(E2:{}{})/C2'.format(get_column_letter(final_data_df.shape[1] + 3), 1 + number_of_rows)
	cv_part_two = '=stdev(E{0}:{1}{2})/C{0}'.format(number_of_rows + 2, get_column_letter(final_data_df.shape[1] + 3), number_of_rows*2 + 1) 
	final_data_df.insert(loc=1, column='Average', 
						value= [average_part_one] + temp_empty + [average_part_two] + temp_empty)#average
	final_data_df.insert(loc=2, column='CV', 
						value= [cv_part_one] + temp_empty + [cv_part_two] + temp_empty)
	column0 = ['total'] + temp_empty + ['excluded'] + temp_empty
	final_data_df.insert(loc=0, column=file_name.split('.')[0], value=column0)
